...
4. **Step 4: Model Registration**: Execute `create_lookml_model` strictly via an inline Code Mode MCP tool call (`call_mcp_tool` or eager equivalent with `ServerName: "lkr_codemode"`, `ToolName: "run_python_code"`, `Arguments: {"code": "...", "dev_mode": true}`) to link the model name to the target project and database connections:
   ...
   ```
5. **Step 5: Conversational Analytics Agent Provisioning** ([`create-conversational-analytics-agent`](../create-conversational-analytics-agent/SKILL.md)): Invoke the `create-conversational-analytics-agent` skill to provision the Looker Conversational Analytics (CA) Agent connected to the `order_items` explore of the target model with standardized markdown prompt instructions, and grant `"view"` access permissions to the `"Embed Demo Users"` content group via `POST /agents`, `GET /content/{terms}`, and `POST /content_metadata_access`.
6. **Step 6: Optional Module Opt-In**: Ask the user if they would like to enable optional plug-and-play modules (such as the Looker GE ADK Agent). If confirmed, execute `python3 scripts/install_module.py --module adk_agent` in the terminal to guide them through BYOA or remote deployment (via root `./deploy.sh`) of CA, MCP, and Advanced ADK agents.
7. **Step 7: Report Success & Output Clickable UI Model Link**: At the end of the onboarding turn, present a clear summary of what was configured and provide a clickable URL linking directly to the newly deployed model file in the Looker UI, formatted exactly like this:
   ```
