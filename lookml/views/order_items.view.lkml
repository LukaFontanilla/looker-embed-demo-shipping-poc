view: order_items {
  ...
  dimension: days_to_process {
    type: number
    sql: CASE
           WHEN ${status} = 'Processing' THEN TIMESTAMP_DIFF(CURRENT_TIMESTAMP(), ${created_raw}, DAY)
           ELSE TIMESTAMP_DIFF(COALESCE(${shipped_raw}, CURRENT_TIMESTAMP()), ${created_raw}, DAY)
         END ;;
    description: "Number of days to process an order item."
  }
  measure: average_days_to_process {
    type: average
    sql: ${days_to_process} ;;
    value_format_name: decimal_1
    description: "Average days taken to process order items."
  }
  dimension: shipping_time {
    type: number
    sql: TIMESTAMP_DIFF(${delivered_raw}, ${shipped_raw}, DAY) ;;
    description: "Number of days between shipped and delivered."
  }
